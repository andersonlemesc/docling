...
def collect_images_and_ocr_results(markdown_content, ocr_processor, min_confidence=0.5):
    ...
    logger.info(
        f"Coletando OCR para {len(matches)} imagens encontradas no documento")
    ...
    for i, match in enumerate(matches):
        ...
        ocr_text = ocr_processor.process_base64_image(
            img_base64, min_confidence)
        ...
        if ocr_text == "[Nenhum texto detectado na imagem]":
            ...
            log_text = ocr_text[:50] + \
                "..." if len(ocr_text) > 50 else ocr_text
...
def apply_enhanced_ocr(conv_result, temp_dir, ocr_language="pt", min_confidence=0.5, include_image_data=False):
    ...
    Returns:
        str: Conteúdo markdown com transcrições OCR e descrições OpenAI
    """
    try:
        ...
        temp_md_file_with_images = os.path.join(
            temp_dir, f"temp_ocr_with_images_{uuid.uuid4()}.md")
        ...
        logger.info(
            f"Gerando markdown final com modo de imagem: {output_image_mode}")
        ...
        # NOVO: Coletar descrições OpenAI das imagens (se existirem)
        ai_descriptions = []
        has_iterate_items = hasattr(conv_result.document, 'iterate_items')

        if has_iterate_items:
            logger.info("Coletando descrições OpenAI das imagens")
            ...
            for element, *_ in conv_result.document.iterate_items():
                if isinstance(element, PictureItem):
                    if hasattr(element, 'annotations') and element.annotations:
                        ...
                        # Log da descrição para depuração
                        short_desc = desc[:50] + \
                            "..." if len(desc) > 50 else desc
                        logger.info(
                            f"Descrição OpenAI encontrada: {short_desc}")
                    else:
                        # Verificar outros atributos para descrição
                        desc_found = False
                        for attr in ['description', 'caption', 'text']:
                            if hasattr(element, attr) and getattr(element, attr):
                                ai_descriptions.append(getattr(element, attr))
                                desc_found = True
                                break

                        if not desc_found:
                            # Placeholder vazio se não houver descrição
                            ai_descriptions.append("")

        logger.info(f"Coletadas {len(ai_descriptions)} descrições OpenAI")
        ...
        if output_image_mode == "placeholder":
            ...
            logger.info(
                f"Encontrados {placeholder_count} placeholders padrão no markdown")

            # Verificar se temos placeholders suficientes
            if placeholder_count < num_ocr_results:
                logger.warning(
                    f"Atenção: há mais resultados OCR ({num_ocr_results}) do que placeholders ({placeholder_count})")
            ...
            for i in range(min(placeholder_count, num_ocr_results)):
                modified_md = modified_md.replace(
            ...
            logger.info(
                f"Encontradas {len(img_matches)} imagens em base64 no markdown final")

            # Verificar se temos imagens suficientes
            if len(img_matches) < num_ocr_results:
                logger.warning(
                    f"Atenção: há mais resultados OCR ({num_ocr_results}) do que imagens ({len(img_matches)})")
        ...
        # Agora, adicionar as transcrições OCR e descrições OpenAI nos placeholders únicos
        logger.info(
            "Adicionando transcrições OCR e descrições OpenAI aos placeholders")
        ...
        for i, ocr_text in enumerate(ocr_results):
            if i < len(unique_placeholders):
                ...
                # Adicionar descrição OpenAI, se disponível
                if i < len(ai_descriptions) and ai_descriptions[i]:
                    result_block += f"<!-- AI Description -->\n> **Descrição da imagem:** {ai_descriptions[i]}\n\n"

                final_md = final_md.replace(placeholder, result_block)

                # Verificar se a substituição funcionou
                if placeholder in final_md:
                    logger.warning(
                        f"Falha ao substituir o placeholder {i+1}: '{placeholder}'")
                else:
        ...
        remaining_placeholders = re.findall(
            r'<!-- ocr-image-\d+ -->', final_md)
        if remaining_placeholders:
            logger.warning(
                f"Encontrados {len(remaining_placeholders)} placeholders não substituídos. Aplicando correção...")

            for placeholder in remaining_placeholders:
                logger.warning(
                    f"Substituindo placeholder restante: {placeholder}")
...
def process_document_with_ocr(conv_result, temp_dir, ocr_language="pt", min_confidence=0.5, include_image_data=True):
    ...
    try:
        logger.info(
            "Processando documento com OCR aprimorado (sem retornar formato específico)")
        ...
        for i, result in enumerate(ocr_results):
            if result and result != "[Nenhum texto detectado na imagem]":
                short_text = result[:30] + \
                    "..." if len(result) > 30 else result
