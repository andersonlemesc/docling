...
async def process_file(
    ...
    include_descriptions: bool = Query(True, description="Inclui descrições de imagens no markdown quando disponíveis"),
    # Parâmetros para escolha do provedor de LLM
    llm_provider: str = Query("ollama", description="Provedor LLM a ser usado: 'ollama', 'openai' ou 'interno'"),
    # Parâmetros para Ollama
    ollama_url: str = Query("http://localhost:11434/v1/chat/completions", description="URL da API Ollama para descrição de imagens"),
    ollama_model: str = Query("llama3.2-vision:latest", description="Modelo Ollama a ser usado para descrição de imagens"),
    # Parâmetros para OpenAI
    openai_api_key: Optional[str] = Query(None, description="Chave da API OpenAI para descrição de imagens"),
    openai_model: str = Query("gpt-4o-mini", description="Modelo OpenAI a ser usado para descrição de imagens"),
    openai_max_tokens: int = Query(500, description="Número máximo de tokens para resposta da OpenAI")
):
    temp_dir = None
    file_path = None
    ...
    try:
        ...
        if generate_images or use_enhanced_ocr:
            ...
            try:
                ...
                logger.warning(f"Erro ao configurar incorporação de imagens: {e}")
        
        # Configurar descrição de imagens usando LLM se solicitado
        if describe_images:
            logger.info(f"Configurando descrição de imagens com provedor: {llm_provider}")
            ...
            if llm_provider.lower() == "ollama":
                ...
                if not openai_api_key:
                    raise ValueError("API key da OpenAI é necessária quando usando o provedor 'openai'")
        ...
        if export_format.lower() == "markdown":
            ...
            try:
                    ...
                    for element, *_ in conv_result.document.iterate_items():
                        if isinstance(element, PictureItem):
                            if hasattr(element, 'annotations') and element.annotations:
                                desc = "\n".join([ann.text for ann in element.annotations])
                                ...
                                for attr in ['description', 'caption', 'text', 'ocr_result']:
                                    if hasattr(element, attr) and getattr(element, attr):
                                        all_descriptions.append(getattr(element, attr))
                                        break
                                else:
                                    all_descriptions.append("Sem descrição disponível")
                
                # Número total de descrições coletadas
                num_descriptions = len(all_descriptions)
                logger.info(f"Coletadas {num_descriptions} descrições de imagens")
                ...
                md_file_with_unique_placeholders = os.path.join(temp_dir, f"with_unique_placeholders.md")
                ...
                if not include_image_data:
                    ...
                    logger.info(f"Encontrados {placeholder_count} placeholders no markdown")
                    
                    # Substituir cada placeholder padrão por um placeholder único
                    for i in range(min(placeholder_count, num_descriptions)):
                        md_content = md_content.replace(placeholder_pattern, unique_placeholders[i], 1)
                else:
                    ...
                    for i, match in enumerate(img_matches):
                        if i < num_descriptions:
                            ...
                            md_content = md_content.replace(img_tag, replacement, 1)
                ...
                if include_descriptions and all_descriptions:
                    for i, placeholder in enumerate(unique_placeholders):
                        if i < len(all_descriptions):
                            ...
                            md_content = md_content.replace(placeholder, desc_text)
                ...
                logger.warning(f"Erro ao processar markdown com placeholders únicos: {e}")
                # Fallback para o método padrão
                try:
                    # Definir o modo de imagem
                    image_mode_value = "embedded" if include_image_data else "placeholder"
                    logger.info(f"Usando image_mode='{image_mode_value}' para markdown (fallback)")
                    ...
                    logger.warning(f"Erro ao criar markdown (fallback): {inner_e}")
            ...
            try:
                # Tentar com parâmetros apropriados
                    ...
                    if descriptions:
                        ...
                        for i, match in enumerate(img_tags):
                            if i < len(descriptions):
                                ...
                                new_html = new_html[:pos] + desc_html + new_html[pos:]
            ...
            logger.info(f"HTML gerado com {len(html_content)} caracteres. Contém imagens: {has_images}")
            ...
            try:
                ...
                dict_content = {"error": "Não foi possível exportar para dict/json"}
            ...
            return JSONResponse(
                status_code=400,
                content={"error": f"Formato de exportação '{export_format}' não suportado."}
            )
